...
function buildPivot(staged: StagedSupplier[]): {
  pivot: PivotRow[];
  suppliers: string[];
} {
  ...
  for (const [, pr] of map) {
    const prices = suppliers
      .map((s) => pr[s] as number | undefined)
...
function SortHeader({
  ...
  return (
    <th
      onClick={() => onSort(col)}
      className="px-3 py-2.5 text-left text-muted-foreground font-medium whitespace-nowrap border-b border-border cursor-pointer select-none hover:text-foreground transition-colors"
    >
...
export default function PricingPage() {
  ...
  const allRows: BidRow[] = useMemo(
    () =>
      staged.flatMap((s) =>
        s.rows.map((r) => normaliseRow({ ...r, supplier: s.supplierName })),
      ),
    [staged],
  );
  const { pivot: pivotRaw, suppliers: pivotSuppliers } = useMemo(
    () => buildPivot(staged),
    [staged],
  );
  ...
  const pivot = useMemo(() => {
    ...
    return [...pivotRaw].sort((a, b) => {
      const av = (a[cmpSortCol] as number) ?? 0,
        bv = (b[cmpSortCol] as number) ?? 0;
  ...
  const kpiCards = useMemo(() => {
    ...
    return {
      ...
      savingsPct:
        prices.length > 1 ? +((1 - sorted[0] / avg) * 100).toFixed(1) : null,
  ...
  const tickerText =
    agentLogs.length > 0
      ? agentLogs
          .slice(0, 4)
          .map((l) => `[${l.agent_id.toUpperCase()}] ${l.message}`)
          .join("   ·   ")
  ...
  useEffect(() => {
    ...
    fetch(`${API}/projects`, { headers: hdrs })
      .then((r) => r.json())
      .then((d) => {
        const l: Project[] = Array.isArray(d)
          ? d
          : (d.projects ?? d.items ?? []);
        setProjects(l);
  ...
  useEffect(() => {
    ...
    fetch(`${API}/files/${projectId}?category=supplier_responses`, {
      headers: hdrs2,
    })
      .then((r) => (r.ok ? r.json() : []))
      .then(
        async (
          ...
          if (!files.length) {
            ...
            setSupplierFiles([]);
            setProjectLoadMsg(
              "No supplier files found. Upload a pricing sheet to get started.",
            );
          ...
          const mapped: SupplierFile[] = files.map((f) => ({
            path: f.id,
            name: ((f as any).display_name ?? f.filename)
              .replace(/\.[^.]+$/, "")
              .trim(),
            filename: f.filename,
            id: f.id,
          ...
          setProjectLoadMsg(
            `Found ${files.length} supplier file${files.length > 1 ? "s" : ""}. Auto-loading bids…`,
          );
          (async () => {
            for (const f of files) {
              try {
                ...
                const rows: any[] =
                  data.line_items ?? data.rows ?? data.items ?? [];
                const rawName =
                  data.supplier_name ??
                  (f as any).display_name ??
                  f.filename ??
                  "Unknown";
                  ...
                  setStaged((prev) => {
                    ...
                    return [
                      ...without,
                      {
                        supplierName: sName,
                        rows,
                        fileName: f.filename ?? (f as any).display_name ?? "",
                        sheetName: "",
                        headerRow: 0,
                      },
                    ];
            ...
            setProjectLoadMsg(
              `Loaded ${files.length} supplier file${files.length > 1 ? "s" : ""}.`,
            );
            setProjectLoading(false);
          })();
        },
  ...
  useEffect(() => {
    const t = setInterval(() => {
      fetch(`${API}/agent-logs?limit=10`, { headers: ah })
        ...
        .then((logs: AgentLog[]) =>
          setAgentLogs(
            logs.filter((l) => ["pricing", "rfp"].includes(l.agent_id)),
          ),
        )
        .catch(() => {});
    }, 3000);
  ...
  useEffect(() => {
    setPage(1);
  }, [pageSize, sortCol, sortDir, staged]);
  ...
  const doIngest = useCallback(
    async (
      ...
      try {
        ...
        if (sname ?? supplierName)
          fd.append("supplier_name", sname ?? supplierName);
        ...
        if (!data?.diagnostics)
          throw new Error("Invalid response from ingest-v2");
        setParsed(data);
  ...
  const getBlob = async (): Promise<{ blob: Blob; fname: string } | null> => {
    if (ingestMode === "project" && selectedFile) {
      try {
        ...
        return {
          blob: await fetch(contentUrl, { headers: ah }).then((r) => r.blob()),
          fname: selectedFile.filename ?? selectedFile.name,
        };
      } catch {
        return null;
      }
  ...
  const confirmRows = async () => {
    ...
    if (projectId && uploadFile) {
      ...
      fetch(`${API}/files/${projectId}?category=supplier_responses`, {
        ...
        .then((r) => r.json())
        .then(
          (files: { id: string; filename: string; display_name: string }[]) => {
            setSupplierFiles(
              files.map(
                (f) =>
                  ({
  ...
  const downloadXLSX = (
    ...
    if (typeof (window as any).XLSX !== "undefined") {
      ...
      sheets.forEach((sh) => {
        const ws = XL.utils.aoa_to_sheet([sh.headers, ...sh.rows]);
        XL.utils.book_append_sheet(wb, ws, sh.name.slice(0, 31));
      });
      XL.writeFile(wb, filename);
    } else {
      sheets.forEach((sh) =>
        downloadCSV(`${sh.name}.csv`, sh.headers, sh.rows),
      );
    }
  };
  ...
  const handleDownload = (fmt: "csv" | "xlsx") => {
    ...
    const abR = sortedAllRows.map((r) => [
      String(r.lineItem ?? ""),
      String(r.supplier ?? ""),
      String(r.category ?? ""),
      String(r.unitOfMeasure ?? ""),
      Number(r.quantity ?? 0),
    ...
    const cR = pivot.map((r) => [
      String(r.item ?? ""),
      ...pivotSuppliers.map((s) => (r[s] != null ? Number(r[s]) : "")),
      r.lowest_price != null ? Number(r.lowest_price) : "",
      String(r.lowest_supplier ?? ""),
      r.highest_price != null ? Number(r.highest_price) : "",
      String(r.highest_supplier ?? ""),
      r.avg_price != null ? +r.avg_price.toFixed(2) : "",
      r.spread_pct != null ? Number(r.spread_pct) : "",
      agentComment(r, pivotSuppliers),
    ]);
    const pName =
      projects.find((p) => p.project_id === projectId)?.name ?? "project";
    ...
    if (fmt === "csv") {
      ...
      downloadXLSX(`${pName}_pricing_analysis_${ts}.xlsx`, [
        ...
        ...(pivot.length
          ? [{ name: "Comparison", headers: cH, rows: cR }]
          : []),
      ]);
    }
  };
  ...
  const removeSupplier = (name: string) =>
    setStaged((prev) =>
      prev.filter(
        (s) => s.supplierName !== name.replace(/\.[^.]+$/, "").trim(),
      ),
    );
  ...
  const previewCols =
    previewRows.length > 0
      ? Object.keys(previewRows[0]).filter((k) => !["id", "delta"].includes(k))
      : [];
    ...
    <div className="flex flex-col h-full min-h-screen bg-background text-foreground">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Compare supplier bids · identify savings · benchmark positions
            </p>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            <div className="relative">
              ...
              <svg
                className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
      ...
      <div className="flex-1 p-6 flex flex-col gap-6 overflow-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            {
              label: "Bids Received",
              value: kpiCards.totalBids,
              sub: "line items",
              cls: "border-primary/20 bg-primary/5 text-primary",
            },
            {
              label: "Suppliers",
              value: kpiCards.suppliers,
              sub: "in comparison",
              cls: "border-accent/20 bg-accent/5 text-accent",
            },
            {
              label: "Potential Savings",
              value:
                kpiCards.savingsPct != null ? `${kpiCards.savingsPct}%` : "—",
              sub: "vs avg baseline",
              cls: "border-success/20 bg-success/5 text-success",
            },
            {
              label: "Lowest Unit Price",
              value: kpiCards.lowestBid != null ? fmt(kpiCards.lowestBid) : "—",
          ...
          <div className="w-72 flex-shrink-0 flex flex-col gap-3">
              ...
              <CardContent className="p-4 flex flex-col gap-3">
                  ...
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">
                      ...
                      {!projectLoading &&
                        projectLoadMsg &&
                        supplierFiles.length > 0 && (
                          <span className="ml-1 text-[10px] text-success opacity-75">
                            ✓
                          </span>
                        )}
                    </label>
                            ...
                            <button
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  const urlRes = await fetch(urlEp, {
                                    headers: ah,
                                  });
                    ...
                    <label className="flex flex-col items-center gap-2 border border-dashed border-border rounded-md p-4 cursor-pointer hover:border-primary/40 bg-background transition-colors">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setUploadFile(f);
                          if (!supplierName)
                            setSupplierName(f.name.replace(/\.[^.]+$/, ""));
                          doIngest(f, f.name, 0);
                        }}
                      />
                    </label>
                ...
                {sheetNames.length > 1 && (
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">
                      Auto-selected best sheet — change if needed.
                    </p>
                  </div>
                )}
                ...
                {parsed && (
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">
                      Header Row{" "}
                      <span className="opacity-60">(0 = first row)</span>
                    </label>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">
                      Increase if file has title rows above headers.
                    </p>
                  </div>
                )}
              </CardContent>
            ...
            <Card>
              ...
              <div
                ...
                className="px-4 pb-1 text-[10px] text-success"
              >
                ✓ Saved to project
              </div>
                  ...
                  staged.map((s, i) => (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-medium border ${SUPPLIER_COLORS[i % SUPPLIER_COLORS.length]}`}
                        >
                          {s.supplierName}
                        </span>
                        <span className="text-xs text-muted-foreground">
                      ...
                      <button
                        ...
                        className="text-muted-foreground/40 hover:text-destructive transition-colors text-xs ml-2"
                      >
                        ✕
                      </button>
                    </div>
              ...
              <Card>
                <CardHeader className="pb-2 flex-row items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge
                      variant="outline"
                      className={`text-xs ${confidenceCls(parsed.diagnostics.parse_confidence)}`}
                    >
                      {parsed.diagnostics.parse_confidence} confidence
                    </Badge>
                    <span className="text-xs text-muted-foreground">
                      {parsed.diagnostics.accepted_line_items} rows accepted
                      {parsed.diagnostics.excluded_rows.length > 0 &&
                        `, ${parsed.diagnostics.excluded_rows.length} excluded`}
                    </span>
                  </div>
                    ...
                    <thead className="sticky top-0 bg-muted z-10">
                      <tr>
                        {previewCols.map((c) => (
                          <th
                            key={c}
                            className="px-3 py-2 text-left text-muted-foreground font-medium whitespace-nowrap border-b border-border"
                          >
                            {c}
                          </th>
                        ))}
            ...
            <Card>
              <CardHeader className="pb-2 flex-row items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {allRows.length > 0
                      ? `${allRows.length} rows · ${staged.length} suppliers`
                      : "No bids loaded — select a project or upload a file"}
                  </span>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  {allRows.length > 0 && (
              ...
              <div className="overflow-auto border-t border-border max-h-[380px]">
                <table className="w-full text-xs min-w-full">
                  <thead className="sticky top-0 bg-muted z-10">
                    <tr>
                      {[
                        { col: "lineItem", label: "Item" },
                        { col: "supplier", label: "Supplier" },
                        { col: "category", label: "Category" },
                        { col: "unitOfMeasure", label: "UoM" },
                        { col: "quantity", label: "Qty" },
                        { col: "unitPrice", label: "Unit Price" },
                        { col: "total", label: "Total" },
                        { col: "delta", label: "Δ vs Best" },
                      ].map(({ col, label }) => (
                        <SortHeader
                          key={col}
                          label={label}
                  ...
                  <tbody>
                    {pagedRows.length === 0 ? (
                      ...
                      pagedRows.map((row, i) => {
                        ...
                        const sIdx = staged.findIndex(
                          (s) => s.supplierName === row.supplier,
                        );
                        return (
                          <tr
                            key={i}
                          ...
                          >
                            ...
                            <td className="px-3 py-2 whitespace-nowrap">
                              <span
                                className={`px-1.5 py-0.5 rounded text-[10px] font-medium border ${SUPPLIER_COLORS[(sIdx >= 0 ? sIdx : 0) % SUPPLIER_COLORS.length]}`}
                              >
                                {String(row.supplier ?? "—")}
                              </span>
                            </td>
              ...
              <div className="flex items-center justify-between px-4 py-2 border-t border-border text-xs text-muted-foreground">
                <span>
                  Showing {Math.min((page - 1) * pageSize + 1, allRows.length)}–
                  {Math.min(page * pageSize, allRows.length)} of{" "}
                  {allRows.length}
                </span>
                <div className="flex gap-1">
                  <button
                    onClick={() => setPage(1)}
                    disabled={page === 1}
                    className="px-2 py-1 rounded border border-border disabled:opacity-30 hover:border-primary/40"
                  >
                    «
                  </button>
                  <button
            ...
            <Card>
              <CardHeader className="pb-2 flex-row items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {pivot.length} SKUs · {pivotSuppliers.length} suppliers
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                ...
                <table className="w-full text-xs min-w-full">
                  <thead className="sticky top-0 bg-muted z-10">
                    <tr>
                      ...
                      {pivotSuppliers.map((s, i) => (
                        <th
                          key={s}
                          className="px-3 py-2.5 text-right whitespace-nowrap border-b border-border"
                        >
                          <span
                            className={`px-2 py-0.5 rounded text-xs font-medium border ${SUPPLIER_COLORS[i % SUPPLIER_COLORS.length]}`}
                          >
                      ...
                      <SortHeader
                        ...
                        onSort={handleCmpSort}
                      />
                      <th className="px-3 py-2.5 text-left text-muted-foreground font-medium border-b border-border min-w-[200px]">
                        Analysis
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {pivot.length === 0 ? (
                      <tr>
                        <td
                          colSpan={pivotSuppliers.length + 6}
                          className="px-4 py-10 text-center text-muted-foreground text-sm"
                        >
                          {projectLoading
                            ? projectLoadMsg
                            : staged.length < 2
                              ? "Add at least 2 suppliers to see a comparison"
                        ...
                        >
                          ...
                          <td className="px-3 py-2 text-right whitespace-nowrap tabular-nums text-destructive">
                            {row.highest_price != null ? (
                              <>
                                {fmt(row.highest_price)}{" "}
                                <span className="text-muted-foreground/50 font-normal text-[10px]">
                                  ({row.highest_supplier})
                                </span>
                              </>
                            ) : (
                              "—"
      ...
      <div className="border-t border-border bg-card py-2 px-4 flex items-center gap-3 overflow-hidden flex-shrink-0">
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <span
            className={`w-1.5 h-1.5 rounded-full ${agentLogs[0]?.status === "running" ? "bg-primary animate-pulse" : "bg-muted-foreground/30"}`}
          />
        ...
        {agentLogs[0] && (
          <Badge
            ...
            className={`flex-shrink-0 text-[10px] ${
              agentLogs[0].status === "complete"
                ? "text-success border-success/30"
                : agentLogs[0].status === "running"
                  ? "text-primary border-primary/30"
                  : agentLogs[0].status === "error"
